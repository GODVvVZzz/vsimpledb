...
public class BTreeLeafPage extends BTreePage {
	...
	public void setLeftSiblingId(BTreePageId id) {
		if(id == null) {
			...
			if(id.getTableId() != pid.getTableId()) {
				System.out.println("table id mismatch in setLeftSiblingId");
				return;
			}
			if(id.pgcateg() != index.BTreePageId.LEAF) {
				System.out.println("leftSibling must be a leaf node");
				return;
			}
	...
	public void setRightSiblingId(BTreePageId id) {
		if(id == null) {
			...
			if(id.getTableId() != pid.getTableId()) {
				System.out.println("table id mismatch in setRightSiblingId");
			}
			if(id.pgcateg() != BTreePageId.LEAF) {
				System.out.println("rightSibling must be a leaf node");
			}
	...
	Tuple getTuple(int i) throws NoSuchElementException {
		...
		try {
			if(!isSlotUsed(i)) {
				//System.out.println(String.format("BTreeLeafPage.getTuple: slot %d in %d:%d is not used", i, pid.getTableId(), pid.getPageNumber()));
				return null;
			}

			//System.out.println(String.format("BTreeLeafPage.getTuple: returning tuple %d", i));
			return tuples[i];
...
class BTreeLeafPageReverseIterator implements Iterator<Tuple> {
	...
	public BTreeLeafPageReverseIterator(index.BTreeLeafPage p) {
		this.p = p;
